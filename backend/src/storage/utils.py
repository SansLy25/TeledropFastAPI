...
async def check_conflicts(parent: int | Folder, name: str, session: AsyncSession):
    if isinstance(parent, int):
        parent = await FolderService.get(session, parent)

    if await FileService.get_by_parent_and_name(
        session, parent, name
    ) or await FolderService.get_by_name_and_parent(session, name, parent.id):
